...
const ProductPage = ({ product, isLeft }) => {
  ...
  // Get current image - supports karat-based images, color-based images, and legacy
  const getCurrentImage = () => {
    // First try karat-based images (new system)
    const karatImages = product.karatImages?.[selectedKarat];
    if (karatImages && karatImages.length > 0) {
      const imageUrl = karatImages[0];
    ...
    // Fallback to color-based images (old system)
      ...
      <div className="w-full h-full flex flex-col p-6 sm:p-10">
        ...
        <div className="space-y-4">
          ...
          <div className="pt-3 border-t border-ivory-300">
            <a
              href={`https://wa.me/905551234567?text=Merhaba, ${product.modelCode} model ${selectedKarat} ayar ürün hakkında bilgi almak istiyorum.`}
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center justify-center gap-2 w-full py-3 bg-[#25D366] hover:bg-[#1fb855] text-white rounded-lg transition-all font-inter text-sm"
